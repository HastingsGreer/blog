...
1) Repeatedly put out calls for grant applications

2) Everyone choses whether to write an application, and whether to use AI (that can one-shot Navier-Stokes existence and uniqueness) to help them write.

3) Judge the applications by some criteria (optionally assisted by AIs that can one-shot Navier Stokes existence and uniqueness)

4) The winning grant recipients use the money to do alignment research, each applicant decides whether to be assisted by AIs (that can one-shot Navier-Stokes existence and uniqueness). 
